...
from src.utils.adobe import ArtLayer, ReferenceLayer

# return self.layout.file.get('additional_cfg', {}).get('nick', None)


class BorderlessBorderMod(BaseTemplate):
    ...
    * Settings
    ...
    def disable_border(self) -> bool:
        setting = self.config.get_bool_setting("FRAME", "Disable.Border", False)
        override = self.layout.file["kwargs"].get("disable_border", None)
    ...
    def border_color(self) -> str:
        ...
        if self.disable_border:
            return "black"
        override = self.layout.file["kwargs"].get("border_color", None)
        if override is not None:
            return override
    ...
    def disable_border_group(self) -> None:
        ...
        if self.disable_border and self.border_group:
...
class BorderlessShowcaseFCATemplate(BorderlessBorderMod, BorderlessVectorTemplate):
    ...
    def fix_rules_text(self) -> None:
        ...
        if layer := self.text_layer_rules:
    ...
    def post_text_methods(self):
        ...
        return [*super().post_text_methods, self.fix_rules_text]
    ...
    def text_layers_transform_front(self) -> None:
        return super(BorderlessVectorTemplate, self).text_layers_transform_front()
...
class BorderlessModernTemplate(BorderlessBorderMod, BorderlessVectorTemplate):
    ...
    @cached_property
    def pt_pinlines_color_map(self) -> dict[str, ColorObject]:
        return {
    ...
    @cached_property
    def neon_color(self) -> str | None:
        setting = self.config.get_setting("COLORS", "Neon.Color", None)
        override = self.layout.file["kwargs"].get("neon", None)
    ...
    @cached_property
    def card_name_group(self) -> LayerSet | None:
    ...
    @cached_property
    def pt_inner_group(self) -> LayerSet | None:
        return psd.getLayerSet("Inner", [self.pt_group, LAYERS.SHAPE])

    @cached_property
    def pt_outer_group(self) -> LayerSet | None:
